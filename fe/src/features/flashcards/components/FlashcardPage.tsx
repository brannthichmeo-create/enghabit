  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <Card className="mb-4 flex min-h-[260px] flex-col items-center justify-center text-center">
        <span className="text-xs font-medium uppercase tracking-wider text-content-muted">{card.topicName}</span>

        <p className="mt-4 text-4xl font-bold tracking-tight text-content">{card.word}</p>
        {card.phonetic && <p className="mt-1.5 text-sm text-content-muted">{card.phonetic}</p>}
        ...
        {revealed ? (
          <div className="mt-6 w-full animate-fade-in border-t border-line pt-5">
            ...
            {card.example && <p className="mt-2 text-sm italic text-content-muted">"{card.example}"</p>}
          </div>
        ) : (
