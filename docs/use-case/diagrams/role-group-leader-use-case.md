...
Quyền riêng của Trưởng nhóm, đặt cạnh các use case nhóm lớp mà mọi thành viên đều có. Trưởng nhóm kế thừa TOÀN BỘ use case của Người học; sơ đồ chỉ vẽ phần nhóm lớp để khỏi lặp lại sơ đồ Người học.

Tổng số use case của vai trò trong tài liệu này: 18.
...
![Vai trò Trưởng nhóm](svg/role-group-leader-use-case.svg)

```plantuml
@startuml role-group-leader-use-case
